...
Network::Network(const std::vector<unsigned>& topology)
{
	...
	for (unsigned layerNum = 0; layerNum < numLayers; layerNum++)
	{
		...
		//With this we got a new layer, now we should fill it ith neurons
		//add a bias neuron in each layer
		for (unsigned neuronNum = 0; neuronNum <= topology[layerNum]; neuronNum++)
		{
			m_layers.back().push_back(Perceptron(numOutputs, neuronNum));
...
void Network::getResults(std::vector<double>& resultsVals) const
{
	resultsVals.clear();
